    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:
        ...
        if training and self.should_explore():
            ...
            for action in actions:
                ...
                # take the action with the maximum q value
                if q > max_q:
                    max_q = q
                    best_action = action
            return best_action
...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, next_action: Optional[A]):
        ...
        if next_action is None:
            # next q value is 0 if the next state is terminal
            next_q = 0
        else:
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_utility_from_features(self, features: Dict[str, float]) -> float:
        # TODO: Complete this function
        # NotImplemented()
        # initialize the maximum q value with negative infinity
        max_q = float('-inf')
        for action in self.weights.keys():
            # calculate the q value from the features
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        if done:
            ...
            # get the next q value
            next_q = self.__compute_utility_from_features(next_features)
        ...
        for feature, value in features.items():
            # update the weight of this feature
            self.weights[action][feature] += self.learning_rate * difference * value
